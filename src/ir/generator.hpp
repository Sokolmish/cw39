...
private:
    ...
    std::optional<IRval> emitNode(std::unique_ptr<IR_Expr> expr);
//    std::optional<IRval> emitNode(std::shared_ptr<IR_Type> ret, std::unique_ptr<IR_Expr> expr);

    template <typename T, typename... Args>
    std::optional<IRval> emitOrEvalNode(std::shared_ptr<IR_Type> ret, Args&& ...args);
    ...
    IRval emitCast(IRval srcVal, std::shared_ptr<IR_Type> const &dst);
